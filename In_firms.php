...
if (!empty($_FILES['logo']['name'])) {
    ...
    $logoExt = pathinfo($_FILES['logo']['name'], PATHINFO_EXTENSION);
      // ✅ VALIDATION HERE (BEFORE upload)
    $allowed = ['jpg','jpeg','png'];
    if (!in_array(strtolower($logoExt), $allowed)) {
        $error = "Invalid logo file type";
    }
if ($_FILES['logo']['size'] > 2 * 1024 * 1024) {
    die("Logo size must be less than 2MB");
}
    $logoName = time() . "_logo." . $logoExt;
...
if (!empty($_FILES['seal']['name'])) {
    ...
    $sealExt = pathinfo($_FILES['seal']['name'], PATHINFO_EXTENSION);
          // ✅ VALIDATION HERE (BEFORE upload)

    $allowed = ['jpg','jpeg','png'];
    if (!in_array(strtolower($sealExt), $allowed)) {
        $error = "Invalid seal file type";
    }
    if ($_FILES['seal']['size'] > 2 * 1024 * 1024) {
    die("Seal size must be less than 2MB");
}
    $sealName = time() . "_seal." . $sealExt;

    move_uploaded_file($_FILES['seal']['tmp_name'], $uploadDir . $sealName);
    $seal = $sealName;
}
    /* ================= PREPARED STATEMENT ================= */
